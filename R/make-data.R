...
question_dump <- "dump_questions_09_07_2018_09_30_55.csv" 
...
q_dump <- file.path(dropbox_path, "input-data", question_dump) %>%
  read_csv(.,
           col_types = cols(
...
flog.info("%s%% of %s forecasts are dropping out because no resolution is available",
          (sum(is.na(user$correct_option))/nrow(user)*100) %>% round(1),
          format(nrow(user), big.mark = ","))
...
flog.info("User forecasts with answer: N = %s",
          format(nrow(user_w_cond), big.mark = ","))
...
write_rds(user_w_cond, file.path(out_dir, "user_forecasts_w_features.rds"))
write_csv(user_w_cond, file.path(out_dir, "user_forecasts_w_features.csv"))

...
user_raw <- read_csv("input-data/dump_user_forecasts_09_07_2018_09_15_12.csv") %>%
...
suppressWarnings(
  q_dump <- read_csv("input-data/dump_questions_09_07_2018_09_30_55.csv",
                     col_types = cols(
...
user %>%
  group_by(condition) %>% 
  summarize(`Average Brier` = mean(brier)) %>%
...
summary(lm(brier ~ turker + sees_chart + sees_model + has_arima, data = user[user$date>="2018-07-15",]))
summary(lm(brier ~ turker + sees_chart + sees_model + has_arima, data = user[user$date>="2018-08-01",]))
